...
describe("auth Zod Validation schemas", () => {
  ...
  describe("resetPasswordSchema", () => {
    ...
    it("should fail for empty email", () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.flatten().fieldErrors.email).toContain("Email обязателен для заполнения");
      }
    });
    ...
    it("should fail for invalid email format", () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.flatten().fieldErrors.email).toContain("Введите корректный адрес электронной почты");
      }
    });
  });

  describe("setNewPasswordSchema", () => {
    it("should validate matching valid passwords", () => {
